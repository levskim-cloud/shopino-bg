...
function defaults(){
  return {
    ...
    theme:{ink:'#2A2A28',tan:'#C2A278',fontPair:'cormorant',logoMode:'svg',logoImg:'',siteName:'SHOPINO',siteTld:'.BG'},
    ...
    cms:{
      ...
      pages:[
        {id:'pg1',title:'Общи условия',show:true,body:'Настоящите общи условия уреждат отношенията между търговеца и клиентите на онлайн магазина.\n\n1. Поръчки се приемат 24/7 през сайта.\n2. Договорът се счита за сключен след потвърждение на поръчката.\n3. Клиентът има право на отказ в 14-дневен срок съгласно ЗЗП.'},
        {id:'pg2',title:'Политика за поверителност',show:true,body:'Обработваме лични данни съгласно Регламент (ЕС) 2016/679 (GDPR).\n\nСъбираме само данните, необходими за изпълнение на поръчката.'},
        {id:'pg3',title:'Доставка и връщане',show:true,body:'Доставяме с Еконт и Спиди до офис, автомат или адрес за 1–2 работни дни.\n\nБезплатна доставка над €50. Връщане — до 14 дни.'},
        {id:'pg4',title:'Контакти',show:true,body:'Имейл: office@shopino.bg\nТелефон: +359 88 000 0000\nАдрес: гр. София'}
      ]
    }
  };
}
...
function publicData(){
  return {
    ...
    cms: DB.cms, theme: DB.theme, seo: DB.seo, banners: DB.banners,
    settings: {
...
const server=http.createServer(async (req,res)=>{
  ...
  try{
    ...
    if(p==='/api/admin/data'&&m==='PUT'){
      const b=await readBody(req);
      for(const k of ['products','promos','cms','theme','seo','banners'])if(b[k])DB[k]=b[k];
